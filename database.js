...
const db = {
  ...
  getAllProposals: () => [...proposals].sort((a,b) => new Date(b.created_at) - new Date(a.created_at)),
...
function formatProperty(p) {
  return {
    ...
    image:    p.image   ? (p.image.startsWith('Research/') ? p.image : '/uploads/' + p.image) : null,
    brochure: p.brochure ? (p.brochure.startsWith('Research/') ? p.brochure : '/uploads/' + p.brochure) : null,
  };
}

module.exports = { db, formatProperty };
