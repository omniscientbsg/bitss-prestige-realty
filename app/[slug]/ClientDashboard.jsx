        ...
        <section className="relative min-h-[90vh] py-20 px-6 flex flex-col justify-center items-center text-center overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center w-full max-w-5xl mx-auto">
            ...
            {client.video_url && (() => {
              ...
              // Detect YouTube
              const ytMatch = rawUrl.match(/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([\w-]{11})/);
              // Detect Vimeo
              const vimeoMatch = rawUrl.match(/vimeo\.com\/(\d+)/);
              
              if (ytMatch) {
                const embedUrl = `https://www.youtube.com/embed/${ytMatch[1]}?autoplay=1&mute=1&loop=1&playlist=${ytMatch[1]}&controls=1&rel=0`;
                return (
                  <div className="w-full max-w-4xl mb-16 rounded-2xl overflow-hidden border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] bg-dark1">
                    <iframe src={embedUrl} className="w-full aspect-video" allow="autoplay; encrypted-media" allowFullScreen title="Portfolio Video" />
                  </div>
                );
              } else if (vimeoMatch) {
                const embedUrl = `https://player.vimeo.com/video/${vimeoMatch[1]}?autoplay=1&muted=1&loop=1`;
                return (
                  <div className="w-full max-w-4xl mb-16 rounded-2xl overflow-hidden border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] bg-dark1">
                    <iframe src={embedUrl} className="w-full aspect-video" allow="autoplay; fullscreen" allowFullScreen title="Portfolio Video" />
                  </div>
                );
              } else {
                // Direct MP4 or other video file
                const resolvedUrl = rawUrl.startsWith('http') ? rawUrl : rawUrl.startsWith('/') ? rawUrl : '/uploads/' + rawUrl;
                return (
                  <div className="w-full max-w-4xl mb-16 rounded-2xl overflow-hidden border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.6)] bg-dark1">
                  ...
                  <div className="grid grid-cols-3 gap-3 pt-6 border-t border-white/5 mb-3">
                    <div>
                      ...
                      <div className="flex flex-col gap-0.5">
                        <span className={p.our_offer ? "text-red-400/80 line-through text-[11px]" : "text-white font-medium text-[15px]"}>
                          {formatAED(p.price_aed)}
                        </span>
                        {p.our_offer && (
                          <div className="text-[#20bd5a] font-bold text-[14px] flex items-center gap-1 mt-0.5 whitespace-nowrap">
                            <span className="text-[8px] uppercase tracking-wider text-[#20bd5a]/80 border border-[#20bd5a]/30 px-1 rounded-sm bg-[#20bd5a]/10 leading-relaxed">Offer</span>
                    ...
                    <div>
                      ...
                      <div className="text-white font-medium text-[15px]">{p.capital_appreciation || p.capital_gain_5yr || 0}%</div>
                    </div>
                    <div className="text-right">
                      <div className="text-xs text-platinum/40 uppercase tracking-widest mb-1">Projected Yield</div>
                      <div className="text-gold font-semibold text-[15px]">{p.gross_yield}%</div>
                    </div>
                  </div>
                  <div className="flex gap-3 text-xs text-platinum/50 mb-4 flex-wrap items-center">
                    {p.handover && <span className="flex items-center gap-1.5 whitespace-nowrap"><CalendarDays className="w-3.5 h-3.5 text-platinum/40" /> {p.handover}</span>}
                    {(p.handover && p.payment_plan_ratio) && <span className="w-1 h-1 rounded-full bg-platinum/20" />}
                    {p.payment_plan_ratio && <span className="flex items-center gap-1.5 whitespace-nowrap"><PieChart className="w-3.5 h-3.5 text-platinum/40" /> {p.payment_plan_ratio}</span>}
                  </div>
            ...
            {portfolio.length > 0 && (
              <div className="border-t border-white/5 p-6 bg-dark3 flex-shrink-0 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setIsMessageModalOpen(true)} className="bg-white/5 hover:bg-white/10 text-white border border-white/10 py-3 rounded-xl flex items-center justify-center gap-2 transition-all text-sm font-medium">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg> Save as PDF
                    </button>
